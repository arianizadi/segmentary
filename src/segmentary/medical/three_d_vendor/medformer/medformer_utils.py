...
class BidirectionAttention(nn.Module):
    def __init__(
        ...
        if proj_type == "linear":
            self.feat_qv = nn.Conv3d(
                feat_dim, self.inner_dim * 2, kernel_size=1, stride=1, padding=0, bias=False
            )
    ...
    def forward(self, feat, semantic_map):
        ...
        map_out = self.rearrange2(
            map_out, d=self.map_size[0], h=self.map_size[1], w=self.map_size[2]
        )
    ...
    def __init__(
        ...
        if proj_type == "linear":
            ...
            self.feedforward = MBConv(
                out_dim, out_dim, expansion=expansion, kernel_size=kernel_size, act=act, norm=norm
            )
...
class SemanticMapFusion(nn.Module):
    def __init__(
        self, in_dim_list, dim, heads, depth=1, norm=nn.BatchNorm3d, attn_drop=0.0, proj_drop=0.0
    ):
        super().__init__()
        ...
        self.fusion = TransformerBlock(
            dim, depth, heads, dim // heads, dim, attn_drop=attn_drop, proj_drop=proj_drop
        )
...
class up_block(nn.Module):
    def __init__(
        ...
        if map_shortcut:
            ...
            self.map_reduction = (
                nn.Identity()
            )  # nn.Conv3d(in_ch, map_dim, kernel_size=1, bias=False)
